...
class RiskSource(BaseModel):
    ...
    category: str = Field(
        ..., description="Source category (cybercriminal, insider, state, etc.)"
    )
    motivation: str = Field(..., description="Primary motivation")
    capability_level: CriticalityLevel = Field(
...
class TargetedObjective(BaseModel):
    ...
    business_impact: CriticalityLevel = Field(
        ..., description="Potential business impact"
    )
    attack_scenarios: list[str] = Field(
        default_factory=list, description="Related attack scenarios"
    )
...
class SecurityMeasure(BaseModel):
    ...
    implementation_cost: CriticalityLevel = Field(
        ..., description="Implementation cost"
    )
    target_threats: list[str] = Field(
        default_factory=list, description="Threat IDs this measure addresses"
    )
...
class Threat(BaseModel):
    ...
    risk_sources: list[str] = Field(
        default_factory=list, description="Associated risk source IDs"
    )
    targeted_objectives: list[str] = Field(
        default_factory=list, description="Associated objective IDs"
    )

    @field_validator("operational_steps")
    @classmethod
    ...
    def likelihood_score(self) -> float:
        ...
        step_mapping = {"Low": 1, "Medium": 2, "High": 3, "Critical": 4}
...
class Settings(BaseModel):
    ...
    likelihood_scale: list[str] = Field(
        default=["One-shot", "Occasional", "Probable", "Systematic"]
    )
